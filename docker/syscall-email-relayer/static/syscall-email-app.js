// app.js - Syscall Email Service Logic (Strict Mode)
...
async function initPrice() {
    try {
        const price = await readOnlySyscall.getServicePrice('email');
        if(priceDisplay) {
...
btn.addEventListener('click', async () => {
    ...
    if (!destination || !subject || !senderName || !content) {
        logToTerminal("INPUT_ERR: All fields (Email, Subject, Name, Content) are required.", "error");
        return;
    }
    ...
    try {
        ...
        logToTerminal("--- INITIATING EMAIL PROTOCOL ---", "system");
        ...
        const result = await syscall.sendEmail(destination, subject, senderName, content);
        ...
        if(btn.innerHTML !== "RETRY") {
             btn.innerHTML = `<span class="btn-text">PAY & SEND EMAIL</span>`;
        }
